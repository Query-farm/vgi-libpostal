...
#
# Assert the release tag matches the version in pyproject.toml, so a `vX.Y.Z`
# release can never publish a wheel built from a mismatched version. Called with
# the tag as $1 (e.g. "v0.1.0"). Dependency-free (no jq / uv required).
set -euo pipefail
